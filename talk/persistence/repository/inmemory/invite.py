...
class InMemoryInviteRepository(InviteRepository):
    ...
    async def find_pending_by_handle(self, handle: Handle) -> Invite | None:
        ...
        for invite in self._invites:
            if (
                invite.invitee_handle.root == handle.root
    ...
    async def save(self, invite: Invite) -> Invite:
        ...
        # Check for duplicate pending invite (create case)
        existing_pending = await self.find_pending_by_handle(invite.invitee_handle)
        if existing_pending:
